...
def parse_arguments():
    ...
    parser.add_argument(
        "-cube", dest="cube_file", required=True, help="Path to the cube file"
    )
    parser.add_argument(
        "-d_range",
        dest="d_range",
...
class DensityCube(object):
    ...
    def _parse_header(self):
        ...
        self.xVoxel, self.xVoxelWidth = int(self.xVoxelLine[0]), float(
            self.xVoxelLine[1]
        )

        self.yVoxelLine = self.header[4].split()
        self.yVoxel, self.yVoxelWidth = int(self.yVoxelLine[0]), float(
            self.yVoxelLine[2]
        )

        self.zVoxelLine = self.header[5].split()  # Correct the index for zVoxel
        self.zVoxel, self.zVoxelWidth = int(self.zVoxelLine[0]), float(
            self.zVoxelLine[3]
        )
        ...
        print(
            f"Atom Info: {self.atom_count}, ({self.atom_x_origin}, {self.atom_y_origin}, {self.atom_z_origin})"
        )
    ...
    def dfdx(self, z, y, x):
        if x < self.zVoxel - 1:
            return (
                self.grid_data[z, y, x + 1] - self.grid_data[z, y, x]
            ) / self.xVoxelWidth
        else:
            return None
    ...
    def dfdy(self, z, y, x):
        if y < self.yVoxel - 1:
            return (
                self.grid_data[z, y + 1, x] - self.grid_data[z, y, x]
            ) / self.yVoxelWidth
        else:
            return None
    ...
    def dfdz(self, z, y, x):
        if z < self.xVoxel - 1:
            return (
                self.grid_data[z + 1, y, x] - self.grid_data[z, y, x]
            ) / self.zVoxelWidth
        else:
            return None
    ...
    def d2f_dx2(self, z, y, x):
        if x < self.zVoxel - 1 and x > 0:
            return (
                self.grid_data[z, y, x + 1]
                - 2 * self.grid_data[z, y, x]
                + self.grid_data[z, y, x - 1]
            ) / (self.xVoxelWidth**2)
        else:
            return None
    ...
    def d2f_dxdy(self, z, y, x):
        if x < self.zVoxel - 1 and y < self.yVoxel - 1:
            return (
                self.grid_data[z, y + 1, x + 1]
                - self.grid_data[z, y - 1, x + 1]
                - self.grid_data[z, y + 1, x - 1]
                + self.grid_data[z, y - 1, x - 1]
            ) / (2 * self.xVoxelWidth * self.yVoxelWidth)
        else:
            return None
    ...
    def d2f_dxdz(self, z, y, x):
        if x < self.zVoxel - 1 and z < self.xVoxel - 1:
            return (
                self.grid_data[z + 1, y, x + 1]
                - self.grid_data[z - 1, y, x + 1]
                - self.grid_data[z + 1, y, x - 1]
                + self.grid_data[z - 1, y, x - 1]
            ) / (2 * self.xVoxelWidth * self.zVoxelWidth)
        else:
            return None
    ...
    def d2f_dydz(self, z, y, x):
        if y < self.yVoxel - 1 and z < self.xVoxel - 1:
            return (
                self.grid_data[z + 1, y + 1, x]
                - self.grid_data[z - 1, y + 1, x]
                - self.grid_data[z + 1, y - 1, x]
                + self.grid_data[z - 1, y - 1, x]
            ) / (2 * self.yVoxelWidth * self.zVoxelWidth)
        else:
            return None
    ...
    def calculate_hessian_for_all_points(self):
        ...
        hessians = np.array(
            [self.calculate_hessian(x_i, y_i, z_i) for x_i, y_i, z_i in zip(x, y, z)]
        )
    ...
    def write_coords_and_density(self, output_file="density_coords.csv"):
        ...
        with open(output_file, "w") as f:
            ...
            for x, y, z, density in zip(
                X.flatten(), Y.flatten(), Z.flatten(), self.grid_data.flatten()
            ):
